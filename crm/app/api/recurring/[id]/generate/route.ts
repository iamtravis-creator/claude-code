...
function computeDates(schedule: RecurringSchedule, lookaheadDays: number): string[] {
  ...
  if (schedule.frequency === 'weekly' || schedule.frequency === 'biweekly') {
    ...
    if (cur < rangeStart) {
      const stepsNeeded = Math.ceil((rangeStart.getTime() - cur.getTime()) / msPerStep);
...
export async function POST(req: NextRequest, { params }: Params) {
  ...
  const { rows } = await db.execute({
    sql: `SELECT rs.*, c.name AS contact_name, s.name AS service_name
  ...
  const schedule = rows[0] as unknown as (RecurringSchedule & { contact_name?: string; service_name?: string }) | undefined;
  if (!schedule) return NextResponse.json({ error: 'Recurring schedule not found' }, { status: 404 });
  if (!schedule.active) return NextResponse.json({ error: 'Schedule is inactive — activate it first' }, { status: 400 });

  const body = await req.json().catch(() => ({})) as { lookahead_days?: number };
  const lookaheadDays = Number(body.lookahead_days) || 56;

  const allDates = computeDates(schedule, lookaheadDays);

  const { rows: existingRows } = await db.execute({
    sql: 'SELECT scheduled_date FROM jobs WHERE recurring_schedule_id = ?',
    args: [id],
  });
  const existingDates = new Set(existingRows.map(r => r.scheduled_date as string));

  const newDates = allDates.filter(d => !existingDates.has(d));

  if (newDates.length === 0) {
    return NextResponse.json({ data: { created: 0, dates: [], message: 'All dates in the window already have jobs.' } });
  }
  ...
  await db.batch(
    newDates.map(date => ({
      sql: `INSERT INTO jobs (id, title, contact_id, property_id, service_id, recurring_schedule_id,
              status, scheduled_date, start_time, estimated_hours, price, notes, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, 'scheduled', ?, ?, ?, ?, ?, ?, ?)`,
      args: [
        randomUUID(), jobTitle,
        schedule.contact_id ?? null, schedule.property_id ?? null,
        schedule.service_id ?? null, schedule.id, date,
        schedule.start_time ?? null, schedule.estimated_hours ?? null,
        schedule.price ?? null, schedule.notes ?? null, now, now,
      ],
    })),
    'write',
  );
